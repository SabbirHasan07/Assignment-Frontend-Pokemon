import {createContext , useReducer, useState ,Reducer,ReactNode, Dispatch} from 'react'
...
const counterReducer = (state:stateModel, action: actionModel) =>{
  switch(action.type){
    ...
    case "decrement":
      return {
        ...state,_count: state._count - 1 as number
      };
    case "reset":
      return {
        ...state, _count: 0 as number 
      };
    default:
      return state;
  }
}
 const AppContextWithReducer:React.FC<{ children : ReactNode  }> = ({ children  })  =>{
  //useReducer use kore          
 // const [stateVal,setStateVal]  = useState<number>(0);
  const [counter,setCounter] = useReducer(counterReducer,{ _count: 0})
  return <IncrementContext.Provider value={{counter, setCounter}}>
